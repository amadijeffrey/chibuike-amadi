...
class ProductCategory extends Component{
  ...
  AddToCart(cartData, e){
   ...
   attributes.forEach( ({ name, items }) => {
       selected[`selected${name}`] = items[0].value
   })
          ...
          {({loading, error, data}) => {
             ...
             return (
               data.category.products.map(({name, gallery, id, prices, inStock, attributes, brand}) => {
                ...
                const cartData = {name, brand, attributes, prices, gallery }
                ...
                return  <Product inStock={inStock} key={name} onClick={() => this.props.history.push(`/${id}`)}>
                  ...
                  <AddToCartButton className='addToCart' onClick={(e) => this.AddToCart(cartData, e) }>
...
export default connect(mapStateToProps, {createCartItem})(ProductCategory);
