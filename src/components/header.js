                      ...
                      return (
                        data.categories.map(({name}) => {
                          return <Text key={name} text={name} category={this.props.selectedCategory}>
                            <Link to={`/${name}`}>
                            <P text={name} category={this.props.selectedCategory} onClick={(e) => this.props.setCategory(e.target.textContent)}>{name}</P>
                            </Link>
                         </Text>
                        })
                      )            
        ...
        <Actions>
          <CurrencySwitch>
              <CurrencyContainer>
                ...
                { 
                 (this.state.isCurrencyDropdown && !this.state.isCartModalOpen) && <Backdrop onClick={() => this.setState({isCurrencyDropdown: false})}>
                  <DropDown>
          ...
          <Cart>
              <svg onClick={() => this.state.isCurrencyDropdown ? null : this.setState(prevState => ({isCartModalOpen: !prevState.isCartModalOpen}))} style={{position:'absolute',right:'10px'}} width="20" height="19" viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
             ...
             {
              (this.state.isCartModalOpen && !this.state.isCurrencyDropdown) && <CartModal close={() => this.setState({isCartModalOpen: false})} />
             }
               <Badge data-testid="counter">{displayNoOfCartItem(this.props.cart)}</Badge>
          </Cart>
        </Actions>
      </Header>
      )
...
export default connect(mapStateToProps, {setSelectedCurrency, setCategory})(HeaderComponent)  
